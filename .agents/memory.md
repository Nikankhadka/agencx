...
- **OpenRouter free-model reality** (2026-07-11, benchmarked 2026-07-29): free models come and go. Query `openrouter.ai/api/v1/models` for `structured_outputs` support rather than hardcoding a model list. Current CI pinned model: `google/gemma-4-26b-a4b-it:free` (ci.yml:133; since 2026-07-12, replaced a chronically-429'd `qwen/qwen3-next`). **NOTE the drift**: `backend/.env` and `.env.example:23` use `nvidia/nemotron-3-super-120b-a12b:free` instead, so CI and the demo run different models. Benchmarked all 4 free structured-output models back to back: only 4 of 14 free models do structured outputs and all 4 advertise `reasoning`; `gpt-oss-20b` FAILS structured output (ValidationError - disqualifying, every route/inspection call is an `extract`); `nemotron-nano-9b` is the SLOWEST despite being smallest (smaller != faster on free tier); `gemma-4-26b` emits **0 reasoning tokens** vs the nemotron's 78-110. Per-call gemma-4 looks better, but an end-to-end run was slower because free-tier queueing swamps the difference - so no swap is justified until a paid/Azure key makes per-call speed the thing actually being measured.
