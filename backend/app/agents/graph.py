...
# D5: customer-facing progress. Emitted per node from _traced below, so the
# customer sees honest narration of what is actually happening instead of an
# unexplained silence while the serial LLM calls run. These are fixed machine
# keys carrying NO model output - the frontend owns the display copy - so
# forwarding them live does not weaken US-060 (inspection still clears every
# word of model prose before any of it is sent). Nodes absent from this map
...
def _traced(name: str, node: Callable[[AgentState], Awaitable[dict[str, Any]]]) -> Any:
    ...
    async def wrapped(state: AgentState) -> dict[str, Any]:
        ...
        with turn.span(name) as span:
            ...
            # Wall-clock per node. The tracer is a no-op unless Langfuse keys are
            # set, so the span attribute alone would measure nothing today; the
            # structured log line is what makes per-node latency observable in
            # the default free-first stack. Both carry the same number, and the
            # request_id contextvar correlates every node of one turn.
            ...
            logger.info(
                ...
                extra={
                    ...
                    # The one scalar worth correlating a duration against; the
                    # rest of _span_attrs stays on the span (free text like
                    # route_reason would just be log noise).
