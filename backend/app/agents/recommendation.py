...
async def run(state: AgentState) -> dict[str, Any]:
    ...
    if violations and state["selections"]:
        redraft_text = await stream_draft(
            ctx.provider, _draft_messages(query, state["selections"], violations)
        )
    ...
    full_text = await stream_draft(ctx.provider, _draft_messages(query, selections))
