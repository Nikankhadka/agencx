...
Supabase/GoTrue signs access tokens with the project's JWT secret (HS256,
``aud=authenticated``). ``verify_token`` in app/shared/auth.py accepts any token
signed that way, so minting a session is signing the same claims GoTrue signs -
no GoTrue session table is needed for the backend bearer flow.
