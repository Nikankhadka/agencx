...
E-3, D-2, F-2 and G-1 all landed. **F-1 (hygiene) has landed too** (merged
from `development`, 2026-08-29): the five specialist nodes the
supervisor-with-tools re-cut left orphaned (`knowledge.py`, `quoting.py`,
...
| Business hub + Business page | BUILT | E-5: the `.bh-row` hub (Booking page, Settings - Stage 2's Schedule/Money/Plan absent, not disabled) and the booking screen: profile show-back plus the public link, derived from the host via `surfaceUrl()`. **E-6 finished the screen** against the prototype: cover photo and platform tiles as link slots in `brand->links`. **M-4 re-cut the hub into three rows** - Business page (`/business/page`), What you offer (`/business/offerings`), Business details (`/business/details`) - and moved the offerings list off the Business page onto its own screen. **M-6 adds the small first-three offering summary before Preview, while keeping links and Share at the end; About data remains preserved but is no longer editable or rendered.** `/business/booking` and `/settings` are gone as paths rather than aliased |
...
| Deploy lane recovery (2026-09-03) | BUILT | Two stacked failures fixed together. (1) The M-1 `offerings` migrations (0022-0025) had reached staging but never the hosted Supabase: the storefront endpoint 500'd (`relation "offerings" does not exist`) and deploy.yml's customer-page smoke test went red on every CI-green push from 2026-08-29. (2) Both VCR repositories hit the hobby 50-image cap (every deployment pushes a commit-tagged image, nothing deletes), so the next deployment failed at the image-push step (`repository has reached the maximum allowed number of images`) and the smoke test ran against a stale origin. Fixed: migrations applied to prod, registry pruned, the Ignored Build Step set via the API (`commandForIgnoringBuildStep`), and `registry-cleanup.yml` added to prune daily to the newest 5 per repository (needs the `VERCEL_TOKEN` repo secret). deploy.md Step 1.3 now says to re-run migrations on every deploy whose branch adds migration files. Development pushes build previews again (staging = production, development = preview, `feat/*` and PRs skipped) - see the Branches table in deploy.md |
| Login OTP email misconfigured on staging (2026-09-04) | BUILT | Hosted Supabase's Auth config (Site URL, URI allow list, Magic Link / Confirm signup email templates) was never set per deploy.md Step 1.5 - `signInWithOtp` mailed a magic link to the Supabase default Site URL (`http://localhost:3000`) instead of a 6-digit code, confirmed via `auth_logs` (`POST /otp` 200 -> `GET /verify` 303) and `auth.users.recovery_sent_at`. No app code changed (`signInWithOtp`/`verifyOtp` in `login/page.tsx` were already correct); fixed by PATCHing the project's Management API config directly - see `fix/hosted-otp-sends-link` and deploy.md Step 1.5, which now gives the literal field values instead of dashboard-click prose. `docker-compose.yml` gained `GOTRUE_MAILER_TEMPLATES_CONFIRMATION` so local dev mirrors both templates hosted now needs. Custom SMTP (Brevo) remains unconfigured - see Known gaps |
...
| F-1 Hygiene | done | `c25ce4b` (cherry-picked via the development sync) |
