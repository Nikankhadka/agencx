...
| Deploy lane recovery (2026-09-03) | BUILT | Two stacked failures fixed together. (1) The M-1 `offerings` migrations (0022-0025) had reached staging but never the hosted Supabase: the storefront endpoint 500'd (`relation "offerings" does not exist`) and deploy.yml's customer-page smoke test went red on every CI-green push from 2026-08-29. (2) Both VCR repositories hit the hobby 50-image cap (every deployment pushes a commit-tagged image, nothing deletes), so the next deployment failed at the image-push step (`repository has reached the maximum allowed number of images`) and the smoke test ran against a stale origin. Fixed: migrations applied to prod, registry pruned to the newest 10 per repository, the Ignored Build Step set via the API (`commandForIgnoringBuildStep`), and `registry-cleanup.yml` added to prune weekly (needs the `VERCEL_TOKEN` repo secret). deploy.md Step 1.3 now says to re-run migrations on every deploy whose branch adds migration files |
