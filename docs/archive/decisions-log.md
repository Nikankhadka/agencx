...
## ADR: Gemini 3.5 Flash becomes the direct primary; gemma still fails over (2026-08-08)

- 2026-08-08 (provider-routing change, local scope): requested switch of the local primary to Google AI. Gemini needs no code - its OpenAI-compatible endpoint (`https://generativelanguage.googleapis.com/v1beta/openai/`) is served by the existing universal `OpenAICompatProvider` (strict json_schema extract + native tool calling on `auto`, neither Z.ai quirk applied). `backend/.env` swapped: `LLM_PROVIDER=openai_compat`, primary block = Google base URL + `gemini-3.5-flash` + Google AI key; fallback block unchanged (OpenRouter gemma free, `LLM_FALLBACK_PROVIDER=openai_compat`). `.env.example` gained the Gemini pairing in the openai_compat examples. CI and infra untouched (their secrets are OpenRouter keys). Two live findings: (1) `gemini-2.5-flash` is listed in the /models API but 404s with "no longer available to new users" - the current flash mainline is `gemini-3.5-flash` (live-probed OK); `gemini-3.6-flash` was rejected as a primary because it spent its whole output budget on hidden reasoning under max_tokens. (2) openai 2.45.0's validate_response_format rejects passing a pydantic BaseModel class into `chat.completions.create(response_format=...)` - a pre-existing regression that broke EVERY strict-mode extract (the whole openai_compat path, not just Gemini). Fixed in openai_base.py: strict extract now sends the explicit `{"type": "json_schema", "json_schema": {name, schema}}` wire dict; pinned by tests/test_llm_extract_wire.py.

## ADR: onboarding copilot switches from tool-calling to structured extraction (2026-08-14)
