...
export function StorefrontSectionsEditor() {
  ...
  return (
    <section className="border-t border-hairline px-gutter py-5">
      ...
      <p className="mt-1 text-meta text-ink-a40">Write the sections customers see below your offers.</p>
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        ...
        <button
          ...
          disabled={sections.reviews.length >= 6}
        ...
        {sections.reviews.map((review, index) => (
          <div key={index} className="relative py-4 pr-9">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <input value={review.author} onChange={(event) => updateReview(index, { author: event.target.value })} placeholder="Customer name" maxLength={120} className="min-w-0 rounded-field bg-surface-container px-3 py-2.5 text-body-sm text-text outline-none focus:ring-1 focus:ring-accent" />
      ...
      <button type="button" onClick={() => void save()} disabled={busy} className="mt-5 rounded-field bg-accent px-4 py-2.5 text-chip font-medium text-text-inverse disabled:opacity-50">
