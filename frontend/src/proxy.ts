 ...
 * (ticket T-005). Tenant-admin has no root page (console pages live at
 * `/conversations` etc, login/signup at their own paths) - without a rewrite,
 * its `/` would silently fall through to the customer surface's
 * `(customer)/page.tsx`, which also resolves to `/` (route groups do not
 * segment URLs). So `/` is rewritten to `/login` for the tenant-admin surface.
 *
 * Platform (T-033) needs a root page and its own `/login` - both of
